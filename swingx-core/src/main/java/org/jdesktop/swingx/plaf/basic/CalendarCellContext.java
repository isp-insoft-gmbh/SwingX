...
/**
 * MonthView specific CellContext. This is internally used by BasisMonthViewUI
 * rendering.
 * 
...
class CalendarCellContext extends CellContext {
     ...
     * hit-detection. As is, these are hard-coded "magic numbers" which must be
     * the same in both the ui-delegate (which does the hit-detection) and here
     * (which returns the default title border)
     * 
     * Added as preliminary fix for #1028-swingx: title border incorrect if
     * box-padding 0
     */
    ...
    public void installContext(JXMonthView component, Calendar value, boolean selected, boolean focused,
    ...
    /**
     * Returns the special color used for flagged days or per weekday or null if
     * none is set, the component or the calendar are null.
     * 
    ...
    protected Border getBorder() {
        ...
        return BorderFactory.createEmptyBorder(getComponent().getBoxPaddingY(), getComponent().getBoxPaddingX(),
